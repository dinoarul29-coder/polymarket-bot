...
def discover_market() -> Optional[dict]:
    ...
    for term in _search_terms():
        ...
        items = data if isinstance(data, list) else (
            data.get("data") or data.get("results") or data.get("markets") or []
        )

        if isinstance(items, dict):
            items = [items]

        log.info(f"Public search '{term}' returned {len(items)} results")
        ...
        for item in items:
            ...
            if cand:
                ...
                log.info(
                    f"SEARCH CANDIDATE id={cand['market_id']} "
                    f"expiry_in={cand['expiry_ts'] - now:.0f}s "
    ...
    for mkt in all_markets:
        ...
        if cand:
            ...
            log.info(
                f"SCAN CANDIDATE id={cand['market_id']} "
